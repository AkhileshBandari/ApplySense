...
export class WorkableAdapter implements BaseAdapter {
  ...
  detect(): boolean {
    return window.location.hostname.includes('workable.com') ||
           document.querySelector('form[data-ui="application-form"]') !== null;
  }
  ...
  extractForm(): DetectedField[] {
    ...
    inputs.forEach((el) => {
      ...
      let label = '';
      const formGroup = element.closest('[data-ui="form-field"]');
      if (formGroup) {
